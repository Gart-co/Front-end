...
if (projectData) {
    // 这里的到的是project页面的数据
    const projectCollections = loadProjectData(JSON.stringify(data));
    console.log(projectCollections);
}

const marketFilePath = "../data/data/marketplaceCollectionData/data/marketplaceCollectionData/marketplaceArtBlocks.json";
const marketData = loadJsonFile(marketFilePath);
if (marketData) {
  // 这里的到的是marketplace页面的数据
  const makertCollections = loadProjectData(JSON.stringify(data));
